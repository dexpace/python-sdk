...
class AsyncHttpxHttpClient:
    ...
    def _build_request(self, request: Request) -> httpx.Request:
        ...
        # ``httpx.AsyncClient`` requires an async byte stream for the
        # request body. Wrap the SDK body's sync ``iter_bytes`` iterator
        # in an async generator so each chunk is yielded through the
